...
class PatchProvider(object):
    ...
    def sample(self, batch_size):
        """
        Samples a batch of patches for training.

        Args:
            batch_size (int): The number of patches to sample.

        Returns:
            tuple: Three torch tensors containing left patches, right patches, and labels.
        """
        ref_patches = []
        pos_patches = []
        neg_patches = []
        ...
        for _ in range(batch_size):
            ref, pos, neg = self.random_patch()
            ref_patches.append(ref)
            pos_patches.append(pos)
            neg_patches.append(neg)
            labels.append(1)  # Positive example (ref and pos)
            labels.append(0)  # Negative example (ref and neg)

        # Convert lists to numpy arrays, then to tensors
        left_patches = torch.tensor(np.array(ref_patches + ref_patches), dtype=torch.float32)
        right_patches = torch.tensor(np.array(pos_patches + neg_patches), dtype=torch.float32)
